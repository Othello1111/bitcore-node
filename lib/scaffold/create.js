...
var BASE_PACKAGE = {
  description: 'A full Bitcoin node build with Bcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'bcore-lib': '^' + bitcore.version,
    'bcore-node': version
  }
};
...
/**
 * Will create a base Bcore Node configuration directory and files.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/bcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a Bcore Node directory, configuration file,
 * bitcoin configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the bcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the bcore-node directory and configuration
