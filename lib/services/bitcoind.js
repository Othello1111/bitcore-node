...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bcore-node. ' +
  ...
  $.checkState(
    ...
    '"server" option is required to communicate to bitcoind from bcore. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that bitcoind is undergoing a reindex. ' +
             ...
             'of bcore-node services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/viacoind.pid';
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      node.client = new BitcoinRPC({
        protocol: 'http',
        host: '127.0.0.1',
        port: self.spawn.config.rpcport || 5222,
...
/**
 * Will retrieve a block as a Bcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
...
/**
 * Will get a transaction as a Bcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
