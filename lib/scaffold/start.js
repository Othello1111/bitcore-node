...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and bcore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/viacoind')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed bcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in bcore-node services directory
 ...
 * @param {Node} node - The Bcore Node instance
 */
